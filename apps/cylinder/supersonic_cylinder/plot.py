...
class Plot(plotFunctions):
    ...
    def extract_flow_variables(self, group):
        ...
        T = self.Minf**2 * 1.4 * p / rho
        return rho, u, v, rhoE, kappa, p, a, M, T

    def main_plot(self, fname, n_levels):
        f, group1 = self.read_file(fname)
        self.x, self.y = self.extract_coordinates()
        rho, u, v, rhoE ,kappa, p, a, M, T = self.extract_flow_variables(group1)
        variables = [rho, u, v, rhoE ,kappa, p, a, M, T]
        names = ["rho", "u", "v", "rho E", "kappa", "p", "a", "M", "T"]

        # Contour plots
        for var, name in zip(variables, names):
            where_are_NaNs = numpy.isnan(var)
            var[where_are_NaNs] = 0
