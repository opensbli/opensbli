...
for i in range(0, N):
    ...
    # Copy over the template wave.py simulation setup file.
    shutil.copy("src/wave.py", directory_name + "/wave.py")
    
    # The number of points will double each time.
    number_of_points = 100*(2**i)
    
    # Replace the number of points and the simulation name in the setup file.
    with open(directory_name + "/wave.py.new", "w") as new:
        with open(directory_name + "/wave.py", "r") as old:
            for line in old:
                new.write(line.replace('NUMBER_OF_POINTS', str(number_of_points)).replace('SIMULATION_NAME', "\"%s\"" % directory_name))
    shutil.move(directory_name + "/wave.py.new", directory_name + "/wave.py")

    # Generate the code
    exit_code = subprocess.call("cd %s; python wave.py" % (directory_name), shell=True)
    if(exit_code != 0):
    ...
    with open(directory_name + "/%s_opsc_code/Makefile.new" % directory_name, "w") as new:
        with open(directory_name + "/%s_opsc_code/Makefile" % directory_name, "r") as old:
            for line in old:
                new.write(line.replace('wave', str(directory_name)))
